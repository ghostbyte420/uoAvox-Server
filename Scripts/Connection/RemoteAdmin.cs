...
namespace Server.RemoteAdmin
{
	public class AdminNetwork
	{
		...
		public static Packet Compress(Packet p)
		{
			...
			{
				var destSize = (int)(length * 1.001) + 10;

				var dest = ArrayPool<byte>.Shared.Rent(destSize);

				try
				{
					var destBuffer = dest.AsSpan();

					if (Compression.Pack(source.AsSpan(0, length), ref destBuffer))
					{
						return new AdminCompressedPacket(destBuffer, length);
					}
				}
				finally
				{
					ArrayPool<byte>.Shared.Return(dest, true);
				}
			}
	...
	{
		public AdminCompressedPacket(ReadOnlySpan<byte> compData, int unCompSize) : base(0x01)
		{
			EnsureCapacity(1 + 2 + 2 + compData.Length);
			m_Stream.Write((ushort)unCompSize);
			m_Stream.Write(compData);
		}
	}
