		...
		private static bool LoadUOP()
		{
			...
			do
			{
				...
				do
				{
					...
					byte[] sourceData = null, destData = null;

					try
					{
						sourceData = ArrayPool<byte>.Shared.Rent(compressedSize);
						...
						if (cmp == 1)
						{
							destData = ArrayPool<byte>.Shared.Rent(decompressedSize);

							var source = sourceData.AsSpan(0, compressedSize);

							if (!Compression.Unpack(source, destData, out decompressedSize))
							{
								continue;
							}
						}
						...
						using var fs = new MemoryStream(destData);
					...
					finally
					{
						if (sourceData != null)
						{
							ArrayPool<byte>.Shared.Return(sourceData, true);
						}

						if (cmp == 1 && destData != null)
						{
							ArrayPool<byte>.Shared.Return(destData, true);
						}
					}
				}
