...
namespace Server.Network
{
	...
	public static class Compression
	{
		...
		[ThreadStatic]
		private static MemoryStream _CompressBuffer, _DecompressBuffer;

		[ThreadStatic]
		private static ZLibStream _Compressor, _Decompressor;

		public static ZLibStream Compressor => _Compressor ??= new(_CompressBuffer ??= new MemoryStream(), CompressionMode.Compress, false);
		public static ZLibStream Decompressor => _Decompressor ??= new(_DecompressBuffer ??= new MemoryStream(), CompressionMode.Decompress, false);

		public static bool Pack(ReadOnlySpan<byte> source, byte[] dest, out int length)
		{
			var buffer = dest.AsSpan();

			var success = Pack(source, ref buffer);
		...
		public static bool Pack(ReadOnlySpan<byte> source, ref Span<byte> dest)
		{
			try
			{
				var comp = Compressor;
				...
				var destLength = (int)_CompressBuffer.Position;

				if (dest.Length > destLength)
				{
					dest = dest.Slice(0, destLength);
				}

				_CompressBuffer.Position = 0;

				return _CompressBuffer.Read(dest) == destLength;
			}
			catch(Exception ex)
			{
				Console.WriteLine("Warning: Compression.Pack threw and exception:");
		...
		public static bool Unpack(ReadOnlySpan<byte> source, byte[] dest, out int length)
		{
			var buffer = dest.AsSpan();

			var success = Unpack(source, ref buffer);
		...
		public static bool Unpack(ReadOnlySpan<byte> source, ref Span<byte> dest)
		{
			try
			{
				var decomp = Decompressor;

				_DecompressBuffer.Position = 0;
				 
				_DecompressBuffer.Write(source);

				_DecompressBuffer.Position = 0;

				var destLength = decomp.Read(dest);
				...
				return dest.Length == destLength;
			}
			catch(Exception ex)
			{
				Console.WriteLine("Warning: Compression.Unpack threw and exception:");
				Console.WriteLine(ex);
