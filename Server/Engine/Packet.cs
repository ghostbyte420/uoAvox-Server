...
namespace Server.Network
{
	...
	public sealed class DisplayGumpPacked : Packet, IGumpWriter
	{
		...
		private void WritePacked(PacketWriter src)
		{
			...
			lock (m_PackBuffers)
			{
				packedBuffer = m_PackBuffers.AcquireBuffer();
			}

			if (packedBuffer.Length < wantLength)
			{
			...
			m_Stream.Write(4 + packedLength);
			m_Stream.Write(length);
			m_Stream.Write(packedBuffer, 0, packedLength);

			lock (m_PackBuffers)
			{
				m_PackBuffers.ReleaseBuffer(packedBuffer);
			}
		}
	}
